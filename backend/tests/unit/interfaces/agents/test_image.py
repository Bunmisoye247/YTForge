...
async def test_image_agent_registers_and_readies_asset() -> None:
    uow = FakeUnitOfWork()
    ...
    scene = Scene(
        ...
        duration_seconds=Decimal("8"), image_prompt="macro shot", created_at=now, updated_at=now,
    )
    ...
    assert assets.items[0].asset_type == AssetType.IMAGE
